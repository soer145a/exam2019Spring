...
var mars = "imgs/050-mars.png"; //hjulene fra nummer et til fem - et er til venstre og fem er til højre
...
function wheelArrayMaker() {
  ...
  for (wheelCounter = 1; wheelCounter < 6; wheelCounter++) {
    var counter = void 0; //counter tæller op til 20
    ...
    for (counter = 1; counter < 21; counter++) {
      ...
      var wheelSelector = eval("wheel_".concat(wheelCounter)); //roundedImageValue bliver skubbet ind i vores wheelSelector, således at vi får 20 variabler i hvert array
...
function divFactory(array) {
  ...
  array.forEach(function (wheel) {
    ...
    wheel.forEach(function (displayCase) {
      ...
      displayCaseDiv.setAttribute("id", "displayCase".concat(displayCaseCounter));
      displayCaseDiv.setAttribute("class", "displayCaseDiv");
      document.querySelector("#wheel".concat(wheelCounterDiv)).appendChild(displayCaseDiv);
    });
  });
  paintWheelDivs();
}
...
var resultsArray = [];
var arrayPositionArray = [];

function selectThree(array) {
  var arrayPosition = Math.floor(Math.random() * 17) + 1; //arrayPosition tager en tilfældig tal op til 18
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "52847" + '/');
